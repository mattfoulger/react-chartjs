module.exports = {
  createClass: function(chartType, methodNames, dataKey) {
    ...
    classData.componentWillReceiveProps = function(nextProps) {
      var keywordChange = true
      var publisherChange = true
      
      if (this.props.data.datasets) {
        if (this.props.data.datasets[0].data) {
          keywordChange = !(this.props.data.datasets[0].data.length == nextProps.data.datasets[0].data.length)
        }
        publisherChange = !(this.props.data.datasets.length == nextProps.data.datasets.length)
      }

      var chart = this.state.chart;
      if (keywordChange || publisherChange) {
        chart.destroy();
